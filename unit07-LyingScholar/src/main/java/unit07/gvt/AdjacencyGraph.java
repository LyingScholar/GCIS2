...
public class AdjacencyGraph<E> implements Graph<E> {
    ...
    protected void visitDSF(Vertex<E> vertex, Set<Vertex<E>> visited){
        for (Vertex<E> neighbour : vertex.getNeighbors()){
            if(!visited.contains(neighbour)){
                visited.add(neighbour);
                visitDSF(neighbour, visited);
            }
        }
    }
    ...
    protected List<E> visitDFPath(Vertex<E> v, Vertex<E> e, Set<Vertex<E>> visited){
        if (v == e){
            ...
            for(Vertex<E> neighbour :v.getNeighbors()) {
                if(!visited.contains(neighbour)){
                    visited.add(neighbour);
                    List<E> path = visitDFPath(neighbour, e, visited);
                    if(path!=null){
